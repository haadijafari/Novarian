...
class RequestPasswordResetTests(APITestCase):
    def setUp(self):
        self.url = reverse('api:request-password-reset')
...
class ResetPasswordTests(APITestCase):
    def setUp(self):
        self.url = reverse('api:reset-password')
        ...
        cache.set(f"verify_email:{self.email}", "999999", timeout=2 * 60)
        cache.set(f"verify_phone:{self.phone}", "888888", timeout=2 * 60)
...
class PasswordResetThrottleTests(APITestCase):
    def setUp(self):
        ...
        cache.set(f"verification_code:{self.email}", "123456", timeout=300)
        cache.set(f"verification_code:{self.phone}", "654321", timeout=300)
