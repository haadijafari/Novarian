...
import { type ratings } from '@/lib/schemas/schemas'
import "./style.css"
import StarIcon from './StarIcon'

const RatingStars = () => {
  const [index, setIndex] = useState<ratings>(1)
  const [prevIndex, setPrevIndex] = useState<ratings | null>(null)
  const [animate, setAnimate] = useState<'animate-right' | 'animate-left' | 'none'>('none')

  // Create a ref to hold an array of the star LI elements
  const starRefs = useRef<(HTMLLIElement | null)[]>([])
  const divRef = useRef<HTMLDivElement | null>(null)

  // calculating x offset to move the animation
  useLayoutEffect(() => {
    const currentStarEl = starRefs.current[index - 1];
    const containerEl = divRef.current;

    // If both the star and container exist, measure and set the CSS variable
    if (currentStarEl && containerEl) {
      const xPosition = currentStarEl.offsetLeft;
      containerEl.style.setProperty('--x', `${xPosition}px`);
    }
  }, [index])

  const handleClick = (starIndex: ratings) => {
    if (animate === 'none') {
      const direction = starIndex > index ? 'animate-right' : 'animate-left'
  ...
  return (
    ...
    >
      <nav dir='ltr' className={`rating ${animate}`}>
        <ul>
          {
            [...Array(5)].map((_, idx) => {
              let CN = ''
              if (index === idx + 1) {
                CN = animate !== 'none' ? 'move-to' : 'current'
              }
              else if (prevIndex === idx + 1)
                CN = animate !== 'none' ? 'move-from' : ''
              return <StarIcon
                ref={el => { starRefs.current[idx] = el }}
                key={idx}
                current={idx + 1 as ratings}
                className={CN}
                onClick={handleClick}
              />
            })
